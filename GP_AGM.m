function [AUCroc,AUCpr,F1,Phi,r,ProbAve,mi_dot_k,output,z]=GP_AGM(B,K, idx_train,idx_test,Burnin, Collections, IsDisplay, Datatype, Modeltype)
%Code for Gamma Process Affilation Graph Model
...
for iter=1:iterMax
    ...
    if strcmp(Datatype, 'Binary')
        ...
        Mall = truncated_Poisson_rnd(Rate+EPS);
        Mnoise = binornd(Mall,EPS./(EPS+Mall));
        M = Mall-Mnoise;
        EPS = randg(sum(Mnoise)+1e-2)./(1e-2+nnz(BTrain_Mask)/2);
    end
    ...
    %sample phi_ik, either 0 or 1
    
    Phir = Phi*sparse(diag(r));
    for i=randperm(N)
        dextemp = find(mi_dot_k(:,i)==0);
        logprob = zeros(length(dextemp),2);
        %             logprob(:,1) = log(Pi(dextemp))  - Phi(i,dextemp)'.*(BTrain_Mask(i,:)*Phir(:,dextemp))';
        %             logprob(:,2) = log(1-Pi(dextemp));
        logprob(:,1) = log(Pi(i))  - (BTrain_Mask(i,:)*Phir(:,dextemp))';
        logprob(:,2) = log(1-Pi(i));
        
        
        %logprob = bsxfun(@minus,logprob,max(logprob,[],2);
        Phi(i,dextemp) = double((1./(1+exp(logprob(:,2)-logprob(:,1)))>rand(length(dextemp),1))');
        Phir(i,:) = Phi(i,:).*r';
    end
    
    
    Pi = betarnd(sum(Phi,2)'+0.01,0.01+K-sum(Phi,2)');
